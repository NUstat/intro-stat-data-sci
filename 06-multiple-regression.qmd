...
Let's now perform the last of the three common steps in an exploratory data analysis: creating data visualizations. Given that the outcome variable `score` and explanatory variable `age` are both numerical, we'll use a scatterplot to display their relationship. How can we incorporate the categorical variable `gender`? By `mapping` the variable `gender` to the `color` aesthetic, thereby creating a *colored* scatterplot. The following code is similar to the code that created the scatterplot of teaching score over "beauty" score in @fig-numxplot1, but with `color = gender` added to the `aes()`thetic mapping.
...
score_model_interaction <- lm(score ~ age + gender + age * gender, data = evals_ch6)
interact_coef <- summary(score_model_interaction)$coefficients 
colnames(interact_coef) <- c("Estimate", "Std. Error", "t value", "p value")
...
#| eval: true
score_model_parallel_slopes <- lm(score ~ age + gender, data = evals_ch6)
parallel_coef <- summary(score_model_parallel_slopes)$coefficients 
colnames(parallel_coef) <- c("Estimate", "Std. Error", "t value", "p value")
...
Unlike in @tbl-regtable-interaction however, we now only have a single slope for age of -0.009. This is because the model specifies that both the female and male instructors have a common slope for age.  \index{regression!multiple linear!parallel slopes model} This is telling us that an instructor who is a year older than another instructor received a teaching score that is on average 0.009 units *lower*. This penalty for aging applies equally for both female and male instructors. 
...
Now let's consider a *multiple regression* model with two numerical explanatory variables. The dataset we'll use is from ["An Introduction to Statistical Learning with Applications in R, (Second Edition)"](https://www.statlearning.com/), an intermediate-level textbook on statistical and machine learning. Its accompanying `ISLR2` R package contains the datasets that the authors apply various machine learning methods to. 
...
debt_model_coeffs <- summary(debt_model)$coefficients
colnames(debt_model_coeffs) <- c("Estimate", "Std. Error", "t value", "p value")
...
interaction_coeffs  <- summary(model_2_interaction)$coefficients
colnames(interaction_coeffs) <- c("Estimate", "Std. Error", "t value", "p value")
...
model_2_parallel_slopes <- lm(average_sat_math ~ perc_disadvan + size, 
                              data = MA_schools)
parallel_coeffs  <- summary(model_2_parallel_slopes)$coefficients
colnames(parallel_coeffs) <- c("Estimate", "Std. Error", "t value", "p value")
...
::: {#exr-ch06-app1}
Using the `Auto` dataset, predict `mpg` using `displacement` and `weight`. 
:::

::: {#exr-ch06-app2}
Consider the `Bikeshare` dataset. It is known that more people tend to bike on the weekend than weekday, so the variable `workingday` is likely useful in predicting total number of bikers (`bikers`). Additionally we assume that the normalized temperature (`temp`) likely impacts the number of bikers.
...
::: {#exr-ch06-app3}
Using the same variables from @exr-ch06-app2, this time fit an interaction model to predict the total number of bikers using temperature and working day. Plot the resulting model and interpret the coefficients.
:::
...
::: {#exr-ch06-adv1}
Determine if the model in @exr-ch06-app2 or @exr-ch06-app3 is better using the RMSE as the evaluation criteria.
:::


::: {#exr-ch06-adv2}
Using the `Auto` dataset, predict `mpg` using any two variables such that the resulting model has a better RMSE than the model in @exr-ch06-app1.
:::