...
Recall from @sec-viz when we looked at plots of temperatures by months in NYC. What does the standard deviation column in the `summary_monthly_temp` data frame tell us about temperatures in New York City throughout the year?
:::
...
::: {#exr-ch03-app1}
Using the `Bikeshare` dataset, compute the average number of bikers at noon (`hr` 12) each month.
:::

::: {#exr-ch03-app2}
Using the `Auto` dataset, create a variable for the power weight ratio (`pwr`) of a car. `pwr` is calculated from `horsepower/weight`. Using code, determine which car has the largest power weight ratio.
:::

::: {#exr-ch03-app3}
Using the `titanic_train` dataset, calculate the total revenue that the ship received from passengers in each class. Store the output in an object called `fare_calc`. Print the contents such that the total revenue is sorted from high to low.
:::

::: {#exr-ch03-app4}
Using the `titanic_train` dataset, calculate the count of men and women that survived and died.
:::
