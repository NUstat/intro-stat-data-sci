...
3.  @fig-flights-key-diagram above showing how the various datasets can be joined will also be useful.

4.  Consider the data wrangling verbs in @tbl-wrangle-summary-table) as your toolbox!
:::
...
a)  To make exploring a data frame easier by removing all variables except the variables of interest from the data frame object
b)  To make exploring a data frame easier by sorting the rows in alphanumeric order
c)  To make exploring a data frame easier by only outputting the variables of interest
d)  To make exploring a data frame easier by filtering out only rows that match a specified criteria
:::
...
::: {#exr-ch03-adv2}
Using the `covid_pop` dataset created in Problem 16, calculate the total weekly `new_confirmed` COVID cases in 2021 for each state. Plot the total weekly cases to determine if the 4 states had a similar trend.
