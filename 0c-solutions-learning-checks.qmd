# Learning check solutions {#sec-lc-sol}
...
**`r paste0("(LC ", chap, ".", (lc <- lc + 1), ")")`** After viewing the @fig-alpha above, give an approximate range of arrival delays and departure delays that occur the most frequently.  How has that region changed compared to when you observed the same plot without the `alpha = 0.2` set in @fig-noalpha?
...
**`r paste0("(LC ", chap, ".", (lc <- lc + 1), ")")`** Recall from Chapter -@sec-viz when we looked at plots of temperatures by months in NYC. What does the standard deviation column in the `summary_monthly_temp` data frame tell us about temperatures in New York City throughout the year?
...
**`r paste0("(LC ", chap, ".", (lc <- lc + 1), ")")`** Looking at @fig-flights-key-diagram, when joining `flights` and `weather` (or, in other words, matching the hourly weather values with each flight), why do we need to join by all of `year`, `month`, `day`, `hour`, and `origin`, and not just `hour`?
...
1. Figure @fig-flights-key-diagram showing how the various datasets can be joined will also be useful. 
1. Consider the data wrangling verbs in Table @tbl-wrangle-summary-table as your toolbox!
...
However, because for certain carriers certain flights have missing `NA` values, the resulting table also returns `NA`'s. We can eliminate these by adding a `na.rm = TRUE` argument to `sum()`, telling R that we want to remove the `NA`'s in the sum. We saw this in Section -@sec-summarize):
