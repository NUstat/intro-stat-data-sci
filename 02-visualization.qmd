...
![Data Visualization with ggplot2 cheatsheat](images/ggplot_cheatsheet-1.png){#fig-ggplot-cheatsheet}
...
### Conceptual {#sec-ex02-conceptual}
...
::: {#exr-ch02-c05}
Consider the following data frame which counts the total number of career wins for three NBA players. Which layer is added to visualize the distribution of the number of wins each `player` had over their career?
...
### Application {#sec-ex02-application}

For @exr-ch02-app1, @exr-ch02-app2, and @exr-ch02-app3 use the `Bikeshare` dataset included in the `ISLR2` package.

For @exr-ch02-app4, use the `titanic_train` dataset included in the `titanic` package

::: {#exr-ch02-app1}
Plot and describe the relationship between temperature (`atemp`) by bikers (`bikers`). Adjust for overplotting if need be.
:::

::: {#exr-ch02-app2}
Use a faceted histogram to compare the center, spread, and shape of the distribution of `bikers` by `weathersit`.
:::

::: {#exr-ch02-app3}
Use a side-by-side boxplot to compare the center, spread, and shape of the distribution of `bikers` by `season`.
:::

::: {#exr-ch02-app4}
Describe and compare the distribution of passengers that `Survived` and how the distribution varies based on `Sex` in the `titanic_train` dataset.
:::

### Advanced {#sec-ex02-advanced}

For @exr-ch02-adv1 and @exr-ch02-adv2, use the `bikeshare_date` dataset created below. The dataset is derived from the `Bikeshare` dataset in the `ISLR2` package. You will learn about data wrangling in the next Chapter.
...
::: {#exr-ch02-adv1}
Plot a linegraph of the number of `bikers` by `date`. Add a layer for the number of `registered` bikers and set the color of this line to "blue". Add a layer for the number of `casual` bikers and set the color of this line to "red". Hint: define the aesthetics within the `geom` layer.

Compare and describe the linegraph.
:::

::: {#exr-ch02-adv2}
Create a linegraph of `pct_casual` by `date`. Clean up the graph by adding the following layers:
...
d)   Change the y-axis label to percents by adding the following layer `scale_y_*(labels = scales::percent)`. Replace the * with the data type. May be useful to check the help documentation.
:::
